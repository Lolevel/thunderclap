...
const RefreshProgressModal = ({ teamId, onComplete, onError }) => {
  ...
  if (!showBackgroundInModal && status !== 'connecting' && status !== 'error') {
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="card max-w-lg w-full">
        ...
        <div className="flex items-center gap-3 mb-6">
          {status === 'completed' || status === 'background' || status === 'background_complete' ? (
          ...
          <div className="flex-1">
            <h3 className="text-xl font-bold text-text-primary">
              {status === 'completed'
                ...
                : status === 'background' || status === 'background_complete'
            ...
            {progress.total_players > 0 && (
              <div>
                <div className="flex items-center justify-between text-sm mb-2">
                  <span className="text-text-muted">Team-Games:</span>
                  <span className="text-text-primary font-semibold">
                    {progress.players_processed} / {progress.total_players} Spieler
                  </span>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-4 bg-success/10 rounded-lg">
                ...
                <p className="text-xs text-text-muted">Team Matches</p>
              </div>
              <div className="text-center p-4 bg-primary/10 rounded-lg">
                <p className="text-2xl font-bold text-primary">
                  {progress.champions_updated}
                </p>
                <p className="text-xs text-text-muted">Champions</p>
              </div>
              {progress.ranks_updated !== undefined && progress.ranks_updated > 0 && (
            ...
            {(status === 'background' || status === 'background_complete') && backgroundProgress.total_players > 0 && (
              <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-blue-300">
                    {backgroundProgress.player_index + 1} / {backgroundProgress.total_players}
                  </span>
                </div>
                <div className="w-full bg-blue-900/30 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${
                        ((backgroundProgress.player_index + 1) / backgroundProgress.total_players) * 100
                      }%`,
                    }}
                  />
                </div>
