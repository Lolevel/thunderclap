...
const DraftAnalysisTab = ({ teamId }) => {
	...
	const { team_champion_pool, favorite_bans, first_pick_priority, side_performance, matches_analyzed } = draftData;
		...
		<div className="space-y-6">
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-text-secondary">
							Basierend auf {matches_analyzed} analysierten Spielen
						</p>
					</div>
					{side_performance && (
						<div className="flex gap-4">
							<div className="text-center">
								<div className="text-sm text-text-muted mb-1">Blue Side</div>
							...
							<div className="text-center">
								<div className="text-sm text-text-muted mb-1">Red Side</div>
								<div className={`text-lg font-bold ${
			...
			<div className="card">
				...
				{team_champion_pool && team_champion_pool.length > 0 ? (
						...
						<table className="w-full">
							<thead>
								<tr className="border-b border-border">
									<th className="text-left py-2 px-3 text-text-muted font-medium">Champion</th>
									<th className="text-left py-2 px-3 text-text-muted font-medium">Spieler</th>
									<th className="text-center py-2 px-3 text-text-muted font-medium">Picks</th>
									<th className="text-center py-2 px-3 text-text-muted font-medium">W-L</th>
									<th className="text-center py-2 px-3 text-text-muted font-medium">Winrate</th>
								</tr>
							</thead>
							<tbody>
					...
					<p className="text-center py-8 text-text-muted">Keine Champion-Daten verfügbar</p>
				)}
			</div>
						...
						{first_pick_priority.map((pick, index) => (
							<div key={index} className="p-4 bg-surface-hover rounded-lg">
								<div className="flex items-center justify-between mb-2">
									<span className="font-bold text-text-primary">{pick.champion}</span>
								...
								<div className="flex items-center justify-between text-sm">
									<span className="text-text-muted">{pick.frequency}x gepickt</span>
									<span className={`font-semibold ${
										pick.winrate >= 50 ? 'text-success' : 'text-error'
									}`}>
				...
				<div className="card">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{['rotation_1', 'rotation_2'].map((rotation, rotIdx) => (
							<div key={rotation} className="p-4 bg-surface-hover rounded-lg">
								...
								{favorite_bans[rotation] && favorite_bans[rotation].length > 0 ? (
									<div className="space-y-2">
										{favorite_bans[rotation].map((ban, idx) => (
											<div key={idx} className="flex items-center justify-between text-sm">
												<span className="text-text-secondary">{ban.champion}</span>
												<span className="text-text-muted">{ban.frequency}x</span>
											</div>
										))}
									</div>
								) : (
									<p className="text-sm text-text-muted">Keine Daten</p>
								)}
							</div>
						))}
					</div>
					<div className="mt-4 p-3 bg-accent/10 border border-accent/20 rounded-lg">
						<p className="text-sm text-text-secondary">
							<strong>Hinweis:</strong> Ban-Rotationen werden aus Draft-Daten extrahiert.
							Falls leer, wurden diese Daten noch nicht von der Riot API abgerufen.
						</p>
					</div>
				</div>
			)}
