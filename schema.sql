...
CREATE TABLE players (
    ...
    summoner_id VARCHAR(100) UNIQUE, -- Nullable due to Riot API bug (Issue #1092, Aug 2025)
...
CREATE TABLE match_participants (
    ...
    role VARCHAR(20),
    lane VARCHAR(20),
    team_position VARCHAR(20), -- Riot's auto-detected position
    kills INTEGER,
    deaths INTEGER,
    assists INTEGER,
    cs_total INTEGER,
...
CREATE TABLE team_stats (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    team_id UUID REFERENCES teams(id) ON DELETE CASCADE,
    stat_type VARCHAR(50), -- 'tournament', 'all'
    games_played INTEGER DEFAULT 0,
