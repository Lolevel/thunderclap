...
class GamePrepRoster(db.Model):
    ...
    scenarios = db.relationship('DraftScenarioNew', back_populates='roster', cascade='all, delete-orphan')
...
class DraftScenarioNew(db.Model):
    """
    Phase 2: Draft Scenarios (NEW VERSION)
    - Belongs to a roster
...
class GamePrepComment(db.Model):
    ...
    scenario = db.relationship('DraftScenarioNew', foreign_keys=[scenario_id], back_populates='comments')
